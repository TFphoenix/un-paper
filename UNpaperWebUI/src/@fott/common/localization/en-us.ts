  ...
  tags: {
    ...
    warnings: {
      ...
      notCompatibleWithDrawnRegionTag:
        "Drawn regions and ${otherCatagory} values cannot both be assigned to the same document's tag",
  ...
  connections: {
    ...
    providers: {
      azureBlob: {
        ...
        invalidSASMessage:
          '"${project.sourceConnection.name}" has no storage account. Please check it\'s SAS token in the Connections page'
      },
      bing: {
      ...
      canvasCommandBar: {
        ...
        warings: {
          drawRegionUnsupportedAPIVersion:
            'Region labeling is not supported with API ${apiVersion}. It will be supported with the release of v2.1-preview.3'
        }
      }
    },
    messages: {
