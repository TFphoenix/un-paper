  ...
  public tableIconFeatureStyler = (feature, resolution) => {
    if (feature.get('state') === 'rest') {
      return new Style({
        image: new Icon({
          opacity: 0.3,
          scale: this.imageMap?.getResolutionForZoom(3) ? this.imageMap.getResolutionForZoom(3) / resolution : 1,
          anchor: [0.95, 0.15],
      ...
      return new Style({
        image: new Icon({
          opacity: 1,
          scale: this.imageMap && this.imageMap.getResolutionForZoom(3) ? this.imageMap.getResolutionForZoom(3) / resolution : 1,
          anchor: [0.95, 0.15],
          anchorXUnits: 'fraction',
          anchorYUnits: 'fraction',
          src:
