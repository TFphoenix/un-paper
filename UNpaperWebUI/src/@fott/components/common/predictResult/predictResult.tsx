...
import { ITag } from 'src/@fott/models/applicationState';
import { getPrimaryGreenTheme } from 'src/@fott/common/themes';
import { strings } from 'src/@fott/common/strings';
import { downloadFile, downloadZipFile, zipData } from 'src/@fott/common/utils';
      ...
      return (
        ...
        >
          <li className="predictiontag-item" style={style}>
            ...
            <div className={'predictiontag-content'}>{this.getPredictionTagContent(item)}</div>
          </li>
  ...
  private triggerCSVDownload = (): void => {
    ...
    items.forEach(item => {
      csvContent += `\n"${item.fieldName}","${item.text ?? ''}",${
        isNaN(item.confidence) ? 'NaN' : (item.confidence * 100).toFixed(2) + '%'
      },${item.page},"[${item.boundingBox}]"`;
    });
    data.push({
