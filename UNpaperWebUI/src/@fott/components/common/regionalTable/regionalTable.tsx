...
> {
  ...
  private displayRegionalTable = regionalTableToView => {
    ...
    if (regionalTableToView?.type === 'array') {
        ...
        return (
          <div>
            <h5 className="mb-4 ml-2 mt-2 pb-1">
              <span style={{ borderBottom: `4px solid ${this.props.tableTagColor}` }}>
      ...
      for (let i = 0; i < colKeys.length + 1; i++) {
        if (i === 0) {
          columnHeaderRow.push(<th key={i} className={'empty_header hidden'} />);
        } else {
      ...
      Object.keys(regionalTableToView?.valueObject).forEach((rowName, index) => {
        ...
        if (regionalTableToView?.valueObject?.[rowName]) {
          Object.keys(regionalTableToView?.valueObject?.[rowName]?.valueObject)?.forEach(
            (columnName, index) => {
              const tableCell =
                regionalTableToView?.valueObject?.[rowName]?.valueObject?.[columnName];
              tableRow.push(
                <td
                  className={'table-cell'}
                  key={index + 1}
          ...
          colKeys.forEach((columnName, index) => {
            tableRow.push(
              <td className={'table-cell'} key={index + 1}>
                {null}
              </td>
            );
          });
  ...
  render() {
    return <>{this.displayRegionalTable(this.props.regionalTableToView)}</>;
  }
}
