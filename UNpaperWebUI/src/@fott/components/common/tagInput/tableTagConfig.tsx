...
export default function TableTagConfig(props: ITableTagConfigProps) {
    ...
    if (props.tableTag?.type === FieldType.Object) {
      if (props.tableTag.visualizationHint === TableVisualizationHint.Vertical) {
        table = {
          ...
          format: FieldFormat.NotSpecified,
          rows: props.tableTag.fields?.map(row => ({
            name: row.fieldKey,
            type: row.fieldType,
            format: row.fieldFormat,
            originalName: row.fieldKey,
            originalFormat: row.fieldFormat,
            originalType: row.fieldType
          })) || [defaultRowOrColumn],
          columns: props.tableTag?.definition?.fields?.map(col => ({
            name: col.fieldKey,
            type: col.fieldType,
            format: col.fieldFormat,
            originalName: col.fieldKey,
            originalFormat: col.fieldFormat,
            originalType: col.fieldType
          })) || [defaultRowOrColumn],
        ...
        table = {
          ...
          type: FieldType.Object,
          format: FieldFormat.NotSpecified,
          rows: props.tableTag?.definition?.fields?.map(row => ({
            name: row.fieldKey,
            type: row.fieldType,
            format: row.fieldFormat,
            originalName: row.fieldKey,
          ...
          columns: props.tableTag?.fields?.map(col => ({
            name: col.fieldKey,
            type: col.fieldType,
            format: col.fieldFormat,
            originalName: col.fieldKey,
  ...
  const currentProjectTags = useSelector<ITag[]>((state: any) => state.currentProject.tags);
  ...
  const [notUniqueNames, setNotUniqueNames] = useState<{ columns: []; rows: []; tags: boolean }>({
    columns: [],
    rows: [],
    tags: false
  });
  const [headersFormatAndType, setHeadersFormatAndType] = useState<
    TableElements.columns | TableElements.rows
  >(table.headerTypeAndFormat);
        ...
        if (idx === currIdx) {
          if (isCompatibleWithType(docCount, col.originalType, type))
            ...
            toast.warn(
              _.capitalize(
                interpolate(
                  strings.tags.regionTableTags.configureTag.errors.notCompatibleTableColOrRowType,
                  { kind: 'column' }
                )
              )
            );
  ...
  const columnListColumns: IColumn[] = [
    ...
    },
    {
      ...
      isResizable: false,
      onRender: (row, index) =>
        headersFormatAndType === TableElements.columns ? (
  ...
  const rowListColumns: IColumn[] = [
    ...
    },
    {
      ...
      isResizable: false,
      onRender: (row, index) =>
        headersFormatAndType === TableElements.rows ? (
          <Customizer {...defaultTheme}>
            <Dropdown
              className="type_dropdown"
              style={{ marginTop: 16 }}
  ...
  function handleTextInput(name: string, role: string, index: number) {
    if (role === TableElements.column) {
      setColumns(
        columns.map((column, currIndex) => (index === currIndex ? { ...column, name } : column))
      );
    } else {
      setRows(rows.map((row, currIndex) => (index === currIndex ? { ...row, name } : row)));
    }
  }
  ...
  useEffect(() => {
    if (tableTagName) {
      const existingTagName = (currentProjectTags as any).find(
        (item: ITag) => item.name === tableTagName.tableName.trim()
      );
  ...
  function save(
    ...
    cleanRows: ITableConfigItem[],
    cleanColumns: ITableConfigItem[]
  ) {
    const [firstLayerFieldsInput, secondLayerFieldsInput] = getFieldsLayersInput(
    ...
    toast.success(
      `Successfully ${props.tableTag ? 'reconfigured' : 'saved'} "${
  ...
  function validateInput() {
    return !(
      ...
      (props.tableTag &&
        notUniqueNames.tags &&
        tableTagName.tableName !== tableTagName.originalTableName) ||
  ...
  function restoreDeletedField(fieldType: TableElements, index: number) {
    ...
    setDeletedFields(
      [...deletedFields]
        .slice(0, index)
        .concat([...deletedFields].slice(index + 1, deletedFields.length))
    );
  }
      ...
      tableBody.push([
        <tr className="compact-row" key={`row-${i}`}>
          <td key={`cell-${i}-0`} className="">
            <div className="flex-center">{deletedFields[i].originalName}</div>
          </td>
          <td key={`cell-${i}-1`} className="">
            <ActionButton
              className="restore-button flex-center"
              onClick={() => {
    ...
    if (table.rows.length !== 0 && table.columns.length !== 0) {
      ...
      for (let i = 0; i < (isRowDynamic ? 2 : rows.length + 1); i++) {
        ...
        for (let j = 0; j < columns.length + 1; j++) {
          if (i === 0 && j !== 0) {
            const columnHeaderWasRenamed =
              props.tableTag && columns[j - 1].name !== columns[j - 1].originalName;
            tableRow.push(
              <th key={`col-h-${j}`} className="header_column">
                {columnHeaderWasRenamed && (
                  <div className="renamed-value">{columns[j - 1].originalName}</div>
                )}
                <div className="original-value">{columns[j - 1].name}</div>
              </th>
            );
          } else if (j === 0 && i !== 0) {
            if (!isRowDynamic) {
              const rowHeaderWasRenamed =
                props.tableTag && rows[i - 1].name !== rows[i - 1].originalName;
              tableRow.push(
                <th key={`row-h-${j}`} className="header_row">
                  {rowHeaderWasRenamed && (
                    <div className="renamed-value">{rows[i - 1].originalName}</div>
                  )}
                  <div className="original-value">{rows[i - 1].name}</div>
                </th>
              );
            }
          } else if (j === 0 && i === 0) {
  ...
  return (
    <Customizer {...dark}>
      <div className="config-view_container">
        <h4 className="header mt-2">
          {props.tableTag ? 'Reconfigure table tag' : 'Configure table tag'}
        </h4>
        <a
          className="document-link mb-1"
          href="https://aka.ms/form-recognizer/docs/table-labeling"
          target="_blnck"
          rel="noopeber bireferrer"
        >
          Learn more about how to use table tags.
        </a>
        ...
        <div className="columns_container mb-4 ml-12px">
          ...
          {deletedColumns?.length > 0 && (
            <div className="mt-3">
              <table className="">
                <tbody>{getDeletedFieldsTable(TableElements.column)}</tbody>
              </table>
            </div>
          )}
        </div>
        {(props.tableTag?.type === FieldType.Object || type === FieldType.Object) && (
          <div className="rows_container mb-4 ml-12px">
            ...
            <PrimaryButton theme={getPrimaryBlueTheme()} className="add_button" onClick={addRow}>
              <FontIcon iconName="Add" className="mr-2" />
              Add row
            </PrimaryButton>
            {deletedRows?.length > 0 && (
              <div className="mt-3">
                <table className="">
                  <tbody>{getDeletedFieldsTable(TableElements.row)}</tbody>
                </table>
              </div>
            )}
          </div>
          ...
          <div className="preview_container  ml-12px">
            ...
            {tableTagName.tableName && (
              <>
                ...
                <span
                  className="tableName-current"
                  style={{
                    borderBottom: props.tableTag ? `4px solid ${props.tableTag.color}` : null
                  }}
                >
            ...
            {type === FieldType.Array && (
              <div className="rowDynamic_message">
                The number of rows is specified when labeling each document.
              </div>
            )}
              ...
              if (!validateInput()) {
                ...
                if (props.tableTag) {
                  ...
                  props.reconfigureTableConfirm(
                    tableTagName?.originalTableName?.trim(),
                    tableTagName?.tableName?.trim(),
                    tableTagToReconfigure['type'],
                    tableTagToReconfigure['format'],
                    tableTagToReconfigure.visualizationHint,
                    deletedColumns,
                    deletedRows,
                    tableTagToReconfigure['rows'],
                    tableTagToReconfigure.columns
                  );
                } else {
