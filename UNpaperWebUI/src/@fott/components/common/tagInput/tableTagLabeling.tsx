...
export default class TableTagLabeling extends React.Component<ITableTagLabelingProps> {
  public state: ITableTagLabelingState = {
    ...
    rows:
      this.props.selectedTag.type === FieldType.Array ||
      this.props.selectedTag?.visualizationHint === TableVisualizationHint.Vertical
        ? this.props.selectedTag.fields
        : this.props.selectedTag.definition.fields,
    columns:
      this.props.selectedTag.type === FieldType.Array ||
      this.props.selectedTag.visualizationHint === TableVisualizationHint.Vertical
        ? this.props.selectedTag.definition.fields
  ...
  public componentDidMount = async () => {
    if (this.props.selectedTag.type === FieldType.Array) {
      const rows = [
        { fieldKey: '#0', fieldType: FieldType.String, fieldFormat: FieldFormat.NotSpecified }
      ];
  ...
  public componentDidUpdate = async (
    prevProps: Readonly<ITableTagLabelingProps>,
    prevState: Readonly<ITableTagLabelingState>
  ) => {
    ...
    return (
      <div className="table-labeling_container">
        ...
        <h5 className="mb-4 table-name">
          <span
            style={{ borderBottom: `4px solid ${this.props.selectedTag.color}` }}
          >{`${strings.tags.regionTableTags.tableLabeling.tableName}: ${this.props.selectedTag.name}`}</span>
        </h5>
        {(this.props.selectedTag.type === FieldType.Object &&
          this.props.selectedTag.fields &&
          this.props.selectedTag.definition.fields) ||
        this.props.selectedTag.definition.fields ? (
        ...
        {this.props.selectedTag.type === FieldType.Array && (
          <div className="add-row-button_container">
            <PrimaryButton
        ...
        <div className="buttons-container">
          ...
          <DefaultButton
            ...
            theme={getPrimaryGreenTheme()}
            onClick={() => {
              this.props.setTagInputMode(TagInputMode.ConfigureTable);
            }}
          >
          ...
          if (i === 0 && j !== 0) {
            tableRow.push(
              <th key={j} className={'column_header'}>
                {columns[j - 1].fieldKey}
              </th>
            );
          } else if (j === 0 && i !== 0) {
            tableRow.push(
              <th key={j} className={`row_header ${isRowDynamic ? 'hidden' : ''}`}>
                {rows[i - 1].fieldKey}
              </th>
            );
          } else if (j === 0 && i === 0) {
            tableRow.push(
              <th key={j} className={`empty_header  ${isRowDynamic ? 'hidden' : ''}`} />
            );
          } else {
            tableRow.push(
              <td
                className={'table-cell'}
                onClick={() => this.handleCellClick(i - 1, j - 1)}
                key={j}
                onMouseEnter={() =>
                  this.handleTableCellMouseEnter(selectedTableTagBody[i - 1][j - 1])
                }
                onMouseLeave={() => this.handleTableCellMouseLeave()}
              >
                {selectedTableTagBody[i - 1][j - 1]?.find(
                  tableRegion => tableRegion.value === ''
                ) && <FontIcon className="pr-1 pl-1" iconName="FieldNotChanged" />}
                {selectedTableTagBody[i - 1][j - 1]
                  ?.map(tableRegion => tableRegion.value)
                  .join(' ')}
              </td>
            );
          }
