...
export class TagInput extends React.Component<ITagInputProps, ITagInputState> {
  ...
  public render() {
    ...
    const selectedTagRef = selectedTag
      ? this.tagItemRefs.get(selectedTag.name)?.getTagNameRef()
      : null;
        ...
        <div className="tag-input">
          <div ref={this.headerRef} className="tag-input-header p-2">
            ...
            <TagInputToolbar
              ...
              onOnlyCurrentPageTags={() =>
                this.setState({ onlyCurrentPageTags: !this.state.onlyCurrentPageTags })
              }
              onShowOriginLabels={(showOriginLabels: boolean) =>
                this.setState({ showOriginLabels })
              }
              onSearchTags={() =>
                this.setState({
                ...
                {this.state.searchTags && (
                  <div className="tag-input-text-input-row search-input">
                    <input
                      ...
                      autoFocus={true}
                      onFocus={() =>
                        this.setState({ selectedTag: null, tagOperation: TagOperationMode.Rename })
                      }
                    />
  ...
  private onRenameTag = (tag: ITag) => {
    const tagOperation =
      this.state.tagOperation === TagOperationMode.Rename
        ? TagOperationMode.None
        : TagOperationMode.Rename;
    this.setState({
      tagOperation
    });
  };
  ...
  private renderTagItems = () => {
    ...
    return props.map(prop => (
      <TagInputItem
        ...
        onLabelEnter={this.props.onLabelEnter as any}
  ...
  private createTagItemProps = (): ITagInputItemProps[] => {
    ...
    if (onlyCurrentPageTags) {
      const labels = this.props.labels
        .filter(item => item.value[0]?.page === this.props.pageNumber)
        .map(item => item.label);
      const tableLabels = this.props.tableLabels
        .filter(item => item.labels[0]?.value[0]?.page === this.props.pageNumber)
        .map(item => item.tableKey);
      const labeledTags = [...labels, ...tableLabels];

      if (labeledTags.length) {
        return tags
          .filter(tag => labeledTags.find(a => a === tag.name))
          .map<ITagInputItemProps>(tag => {
            return {
              ...
              isRenaming:
                selectedTag &&
                isNameEqual(selectedTag.name, tag.name) &&
  ...
  private onTagItemClick = (tag: ITag, props: ITagClickProps) => {
    if (props.ctrlKey && this.props.onCtrlTagClick) {
      ...
      const tagOperation = showContextualMenu
        ? TagOperationMode.ContextualMenu
        : TagOperationMode.None;
      this.setState({
        selectedTag: tag,
        tagOperation
      ...
      const newTagOperation = showColorPicker
        ? TagOperationMode.ColorPicker
        : TagOperationMode.None;
      this.setState({
      ...
      if (selectedRegions && selectedRegions.length && onTagClick) {
        ...
        if (
          (tag.type === FieldType.Object || tag.type === FieldType.Array) &&
          this.props.selectedRegions?.length
        ) {
          this.props.handleLabelTable(TagInputMode.LabelTable, tag);
          deselect = false;
        } else if (
          labelAssigned &&
          (category === FeatureCategory.DrawnRegion) !== isTagLabelTypeDrawnRegion
        ) {
          if (category === FeatureCategory.Checkbox && isTagLabelTypeDrawnRegion) {
            toast.warn(
              interpolate(strings.tags.warnings.notCompatibleWithDrawnRegionTag, {
                otherCatagory: FeatureCategory.Checkbox
              })
            );
          ...
          if (
            tagCategory === FeatureCategory.Checkbox &&
            !(category === FeatureCategory.Checkbox || category === FeatureCategory.DrawnRegion)
          ) {
  ...
  private getContextualMenuItems = (): IContextualMenuItem[] => {
    ...
    const menuItems: IContextualMenuItem[] = [
      {
        ...
        submenuIconProps: {
          iconName: !(tag.type === FieldType.Object || tag.type === FieldType.Array)
            ? 'ChevronRight'
            : ''
        }
      },
      {
        ...
        submenuIconProps: {
          iconName:
            tag.type !== FieldType.Object && tag.type !== FieldType.Array ? 'ChevronRight' : ''
        }
      },
      {
      ...
      },
      {
        ...
        onClick: this.onMenuItemClick as any
      },
      {
    ...
    return tag.type === FieldType.Object || tag.type === FieldType.Array
      ? menuItems
      : menuItems.filter(item => item.key !== 'reconfigureTable');
    // return menuItems;
  };
  ...
  private isTypeCompatibleWithTag = (tag, type) => {
    ...
    if (
      tag &&
      tag.documentCount <= 0 &&
      tag.type !== FieldType.Object &&
      tag.type !== FieldType.Array
    ) {
      return true;
    }
