...
import { PredictService } from 'src/@fott/services/predictService';
...
export class EditorPage extends React.Component<IEditorPageProps, IEditorPageState> {
  ...
  private tagInputRef: React.RefObject<TagInput>;
  ...
  public render() {
    ...
    const labels =
      (selectedAsset && selectedAsset.labelData && selectedAsset.labelData.labels) || [];
    const tableLabels =
      (selectedAsset && selectedAsset.labelData && selectedAsset.labelData.tableLabels) || [];
              ...
              <div>
                ...
                >
                  {this.state.isRunningOCRs ? (
                    ...
                    'Run Layout on unvisited documents'
                  )}
                </PrimaryButton>
              </div>
            )}
  ...
  private handleLabelTable = (
    ...
    if (
      this.state.selectedTableTagToLabel?.name === selectedTableTagToLabel?.name &&
  ...
  private getTagFromKeyboardEvent = (event: KeyboardEvent): ITag => {
    ...
    if (
      index >= 0 &&
      index < tags.length &&
      (tags[index].type !== FieldType.Array || tags[index].type !== FieldType.Object)
    ) {
      return tags[index];
    }
    return null;
  ...
  private handleTagHotKey = (event: KeyboardEvent): void => {
    ...
    if (tag && selection.length) {
      ...
      const isTagLabelTypeDrawnRegion = this.tagInputRef.current.labelAssignedDrawnRegion(
        labels,
        tag.name
      );
  ...
  private onAssetMetadataChanged = async (assetMetadata: IAssetMetadata): Promise<void> => {
    ...
    if (
      ...
      if (
        ...
        hasTableLabels &&
        assetMetadata.labelData.tableLabels.findIndex(item => item.labels?.length > 0) >= 0
      ) {
        asset.state = AssetState.Tagged;
      } else {
  ...
  private selectAsset = async (asset: IAsset): Promise<void> => {
    ...
    assetMetadata.regions =
      this.canvas.current?.convertLabelDataToRegions(assetMetadata.labelData) || [];
  ...
  private reconfigureTable = async (
    ...
    if (selectedAsset) {
      selectedAsset.regions =
        this.canvas.current?.convertLabelDataToRegions(selectedAsset.labelData) || [];
      this.setState(
        {
          selectedAsset,
          selectedTableTagToLabel: null,
    ...
    this.setState({ tagInputMode: TagInputMode.Basic, reconfigureTableConfirm: false }, () =>
      this.resizeCanvas()
    );
    this.resizeCanvas();
  };
          ...
          async assetId => {
            ...
            if (asset && (asset.state === AssetState.NotVisited || runForAll)) {
              try {
                ...
                const ocrResult = await ocrService.getRecognizedText(
                  asset.path,
                  asset.name,
                  asset.mimeType,
                  undefined,
  ...
  private runAutoLabelingOnNextBatch = async (batchSize: number) => {
    ...
    if (this.state.assets) {
      ...
      for (
        let i = 0;
        i < this.state.assets.length && unlabeledAssetsBatch.length < batchSize;
        i++
      ) {
        ...
        await throttle(
          ...
          async asset => {
            try {
              ...
              const assetMetadata = await assetService.getAssetPredictMetadata(
                asset,
                predictResult
              );
