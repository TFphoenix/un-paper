...
import { ILoadFileHelper, ILoadFileResult, LoadFileHelper } from '../../../common/LoadFileHelper';
import { ITableHelper, ITableState, TableHelper } from '../../../common/tableHelper';
...
export default class PredictPage extends React.Component<IPredictPageProps, IPredictPageState> {
  ...
  public async componentDidUpdate(_prevProps: IPredictPageProps, prevState: IPredictPageState) {
    ...
    if (
      this.props.project?.predictModelId &&
    ...
    if (this.state.file) {
      ...
      if (
        prevState.highlightedTableCellColumnKey !== this.state.highlightedTableCellColumnKey ||
        prevState.highlightedTableCellRowKey !== this.state.highlightedTableCellRowKey
      ) {
        this.setPredictedFieldTableCellHighlightStatus(
          this.state.highlightedTableCellRowKey,
          this.state.highlightedTableCellColumnKey
        );
      }
    }
  }
  ...
  public render() {
    const mostRecentModel =
      this.props.project?.recentModelRecords?.[this.state.selectedRecentModelIndex];
    ...
    return (
      ...
      >
        ...
        <div
          className={'predict-sidebar bg-lighter-1'}
                  ...
                  <>
                          ...
                          <tbody>
                            <tr>
                              <td className="model-selection-info-header p-0">
                                ...
                                <span
                                  title={mostRecentModel.modelInfo.modelId}
                                  className="model-selection-info-value"
                                >
                            ...
                            <tr>
                              {mostRecentModel.modelInfo.modelName && (
                                <td className="model-selection-info-header p-0">
                                  ...
                                  <span
                                    title={mostRecentModel.modelInfo.modelName}
                                    className="model-selection-info-value"
                                  >
                                    {mostRecentModel.modelInfo.modelName}
                                  </span>
                    ...
                    <div className="p-3" style={{ marginTop: '8px' }}>
                      ...
                      {this.props.project.apiVersion === constants.prebuiltServiceVersion &&
                        this.props.match.path !== '/projects/:projectId/predict' && (
                    ...
                    <div className="p-3" style={{ marginTop: '8px' }}>
                      <div className="container-items-end predict-button">
                        <PrimaryButton
                          ...
                          aria-label={
                            !this.state.predictionLoaded ? strings.predict.inProgress : ''
                          }
                          allowDisabledFocus
            ...
            {this.state.viewRegionalTable && (
              <div className="m-2">
                ...
                <PrimaryButton
                  ...
                  onClick={() => this.setState({ viewRegionalTable: false })}
                >
                  Back
                </PrimaryButton>
              </div>
  ...
  private async triggerDownload(): Promise<any> {
    axios
      ...
      .then(response => {
        const modelID =
          this.props.project?.recentModelRecords?.[this.state.selectedRecentModelIndex].modelInfo
            .modelId;
        if (!modelID) {
          throw new AppError(
        ...
        const analyzeScript = response.data.replace(
          /<endpoint>|<subscription_key>|<model_id>|<API_version>/gi,
          (matched: string) => {
            switch (matched) {
              case '<endpoint>':
                return endpointURL;
        ...
        const fileURL = window.URL.createObjectURL(new Blob([analyzeScript]));
  ...
  private async getPrediction(): Promise<any> {
    ...
    let endpointURL = url.resolve(
      ...
      `${interpolate(constants.apiModelsPath, {
        apiVersion
      })}/${modelID}/analyze?includeTextDetails=true`
    );
  ...
  private createBoundingBoxVectorFeature = (text, boundingBox, imageExtent, ocrExtent) => {
    ...
    const tag = this.props.project.tags.find(
      tag => tag.name.toLocaleLowerCase() === text.toLocaleLowerCase()
    );
    const isHighlighted =
  ...
  private createBoundingBoxVectorFeatureForTableCell = (
    text,
    boundingBox,
    imageExtent,
    ocrExtent,
    ...
    const isHighlighted =
      text.toLocaleLowerCase() === this.state.highlightedField.toLocaleLowerCase() ||
      (this.state.highlightedTableCellRowKey === rowKey &&
        this.state.highlightedTableCellColumnKey === columnKey);
    feature.setProperties({
      color: _.get(tag, 'color', '#333333'),
      fieldName: text,
      isHighlighted,
  ...
  private drawPredictionResult = (): void => {
    ...
    const ocrForCurrentPage: any = this.getOcrFromAnalyzeResult(this.state.analyzeResult)[
      ...
      if (field?.type === 'object') {
        Object.keys(field?.valueObject).forEach((rowName, rowIndex) => {
          if (field?.valueObject?.[rowName]) {
            Object.keys(field?.valueObject?.[rowName]?.valueObject).forEach(
              (columnName, colIndex) => {
                ...
                if (tableCell?.page === this.state.currentPage) {
                  ...
                  const feature = this.createBoundingBoxVectorFeatureForTableCell(
                    text,
                    boundingbox,
                    imageExtent,
                    ocrExtent,
    ...
    const checkSucceeded = (resolve, reject) => {
      ...
      ajax.then(response => {
        if (response.data.status.toLowerCase() === constants.statusCodeSucceeded) {
          ...
          reject(
            _.get(response, 'data.analyzeResult.errors[0]', 'Generic error during prediction')
          );
  ...
  private getPredictionsFromAnalyzeResult(analyzeResult: any) {
    const fields = _.get(
      analyzeResult?.analyzeResult ? analyzeResult?.analyzeResult : analyzeResult,
      'documentResults[0].fields',
      {}
    );
    return fields;
  }
  ...
  private onAddAssetToProjectClick = async () => {
    if (this.state.file) {
      const fileName = `${this.props.project.folderPath}/${decodeURIComponent(
        this.state.file.name
      )}`;
      const asset = Object.values(this.props.project.assets).find(asset => asset.name === fileName);
      if (asset) {
        const confirmDuplicatedAssetNameMessage = interpolate(
          strings.predict.confirmDuplicatedAssetName.message,
          { name: decodeURI(this.state.file.name) }
        );
        this.setState({
  ...
  private onAddAssetToProject = async () => {
    if (this.state.file) {
      ...
      await projectActions.addAssetToProject(
        this.props.project,
        fileName,
        fileData,
  ...
  private onTablePredictionClick = (predictedItem: ITableResultItem, tagColor: string) => {
    this.setState({
      viewRegionalTable: true,
      regionalTableToView: predictedItem,
      tableTagColor: tagColor
    });
  };
  ...
  private setPredictedFieldTableCellHighlightStatus = (
    highlightedTableCellRowKey: string,
    highlightedTableCellColumnKey: string
  ) => {
    const features = this.imageMap.getAllFeatures();
    for (const feature of features) {
      if (
        highlightedTableCellRowKey &&
        highlightedTableCellColumnKey &&
        feature.get('rowKey')?.toLocaleLowerCase() ===
          highlightedTableCellRowKey?.toLocaleLowerCase() &&
        feature.get('columnKey')?.toLocaleLowerCase() ===
          highlightedTableCellColumnKey?.toLocaleLowerCase()
      ) {
        feature.set('isHighlighted', true);
      } else {
        feature.set('isHighlighted', false);
  ...
  private async updateRecentModels(project) {
    ...
    project = await projectActions.saveProject(
      {
        ...project,
        recentModelRecords
      },
  ...
  private onMouseEnter = (rowName: string, columnName: string) => {
    this.setState({
      highlightedTableCellRowKey: rowName,
      highlightedTableCellColumnKey: columnName
    });
  };
