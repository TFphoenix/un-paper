...
export default class TrainChart extends React.PureComponent<ITrainChartProps, ITrainChartState> {
  ...
  private getLabels = (): string[] => {
    return Object.entries(this.props.accuracies).map(entry => entry[0] + ' (' + entry[1] + ')');
  };
  ...
  private getColor = (): string[] => {
    return Object.keys(this.props.accuracies).map(label => {
      const tag = this.props.projectTags.find(tag => label === tag.name);
