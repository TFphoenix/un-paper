...
export default class TrainPage extends React.Component<ITrainPageProps, ITrainPageState> {
  ...
  constructor(props) {
    ...
    this.state = {
      inputtedLabelFolderURL:
        ...
        (this.props.project?.folderPath ? '/' + this.props.project.folderPath : ''),
  ...
  public render() {
    ...
    const localFileSystemProvider: boolean =
      this.props.project &&
      this.props.project.sourceConnection &&
      this.props.project.sourceConnection.providerType === 'localFileSystemProxy';
    const trainDisabled: boolean =
      localFileSystemProvider &&
      (this.state.inputtedLabelFolderURL.length === 0 ||
    ...
    return (
      <div className="train-page skipToMainContent" id="pageTrain">
        <main className="train-page-main">
          {currTrainRecord && (
            <div>
              <h2 className="unp-primary"> Train Result </h2>
        ...
        <div className="train-page-menu bg-lighter-1">
          <div className="condensed-list">
            <div className="condensed-list-body">
              <div className="m-3">
                <h2 className="text-shadow-none unp-primary"> Train a new model </h2>
                {localFileSystemProvider && (
                  <div>
                    <span>{strings.train.labelFolderTitle}</span>
                    <TextField
                ...
                <span>{strings.train.modelNameTitle}</span>
                <TextField
                  theme={getGreenWithWhiteBackgroundTheme()}
                ...
                {!this.state.isTraining ? (
                  <div className="container-items-end">
                    ...
                    >
                      ...
                      <h3 className="d-inline text-shadow-none ml-2 mb-0">
                        {strings.train.title}{' '}
                      </h3>
                    </PrimaryButton>
                  </div>
              ...
              <div
                className={!this.state.isTraining ? 'training-record' : 'training-record greyOut'}
              >
                {currTrainRecord && (
                  <>
                    <TrainPanel
                    ...
                    >
                      <FontIcon iconName="Download" style={{ fontWeight: 600 }} />
                      <h3 className="d-inline text-shadow-none ml-2 mb-0">
    ...
    this.trainProcess()
      .then(async trainResult => {
        ...
        for (const asset of assets) {
          ...
          if (
            metadata.labelData &&
            metadata.labelData.labels?.findIndex(label => label.value?.length > 0) >= 0 &&
        ...
        // );
        await projectActions.saveProject(
          { ...trainResult[1], assets: newAssets },
          this.props,
          false,
          false
        );
        this.setState((prevState, props) => ({
          isTraining: false,
          trainMessage: this.getTrainMessage(trainResult[0]),
  ...
  private async trainProcess(): Promise<any> {
    try {
      ...
      const trainStatusRes = await this.getTrainStatus(trainRes.headers['location']);
      const updatedProject = this.buildUpdatedProject(this.parseTrainResult(trainStatusRes));
      await projectActions.saveProject(updatedProject, this.props, false, false);

      return [trainStatusRes, updatedProject];
    } catch (error) {
      const isOnPrem =
        isElectron && this.props.project.sourceConnection.providerType === 'localFileSystemProxy';
      this.setState({
        showTrainingFailedWarning: true,
        trainingFailedMessage: isOnPrem
          ? interpolate(strings.train.errors.electron.cantAccessFiles, {
  ...
  private async cleanLabelData() {
    ...
    for (const asset of assetValues) {
      const assetMetadata: IAssetMetadata = _.cloneDeep(
        await projectActions.loadAssetMetadata(this.props.project, asset)
      );
      let isUpdated = false;
      ...
      if (isUpdated) {
        await projectActions.saveAssetMetadataAndCleanEmptyLabel(this.props.project, assetMetadata);
      }
    }
  }
  ...
  private async getTrainStatus(operationLocation: string): Promise<any> {
    ...
    const extendedTimeoutInMs =
      timeoutPerFileInMs * Object.keys(this.props.project.assets || []).length;
    const res = this.poll(
      () => {
  ...
  private async triggerJsonDownload(): Promise<any> {
    ...
    const currModelUrl =
      ...
      interpolate(constants.apiModelsPath, { apiVersion }) +
      '/' +
      this.state.currTrainRecord.modelInfo.modelId;
    ...
    const fileURL = window.URL.createObjectURL(new Blob([modelJSON]));
  ...
  private async getModelsJson(project: IProject, modelUrl: string) {
    const baseURL = url.resolve(project.apiUriBase, modelUrl);
    const config = {
      headers: {
    ...
    try {
      return await ServiceHelper.getWithAutoRetry(baseURL, config, project.apiKey as string).then(
