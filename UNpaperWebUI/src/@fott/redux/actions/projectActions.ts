...
//   project: IProject
// ): (dispatch: Dispatch, getState: () => IApplicationState) => Promise<IAsset[]> {
//   return async (dispatch: Dispatch, getState: () => IApplicationState) => {
//     const assetService = new AssetService(project);
//     const assets = await assetService.getAssets();
...
export async function loadAssets(project: IProject): Promise<IAsset[]> {
  const assetService = new AssetService(project);
  const assets = await assetService.getAssets();
  let shouldAssetsUpdate = false;
  for (const asset of assets) {
  ...
  if (!areAssetsEqual(assets, project.assets)) {
    loadProjectAssetsAction(assets);
  }
  if (shouldAssetsUpdate) {
    // const { currentProject } = getState();
    // await AssetService.checkAndUpdateSchema(currentProject);
    await AssetService.checkAndUpdateSchema(project);
  }
  return assets;
}
 ...
 */
// REMEMBER: EXPERIMENTAL VERSION
export async function loadAssetMetadata(project: IProject, asset: IAsset): Promise<IAssetMetadata> {
  const assetService = new AssetService(project);
  const assetMetadata = await assetService.getAssetMetadata(asset);
  // dispatch(loadAssetMetadataAction(_.cloneDeep(assetMetadata)));
  loadAssetMetadataAction(_.cloneDeep(assetMetadata));
...
export async function updateProjectTag(
  ...
  if (oldTag.name !== newTag.name) {
    ...
    // Save updated assets
    await assetUpdates.forEachAsync(async assetMetadata => {
      await saveAssetMetadata(project, assetMetadata);
    });
  }
...
//             newDefinitionFields || (tag as ITableTag).definition.fields;
...
// REMEMBER: EXPERIMENTAL VERSION
export async function reconfigureTableTag(
  project: IProject,
  originalTagName: string,
  tagName: string,
  tagType: FieldType,
 ...
 */
export const deleteProjectAssetAction = createPayloadAction<IDeleteProjectAssetAction>(
...
export const updatedAssetMetadataAction = createPayloadAction<IUpdateTagDocumentCount>(
