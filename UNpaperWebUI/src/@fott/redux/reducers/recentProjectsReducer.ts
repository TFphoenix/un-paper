  ...
  switch (action.type as any) {
    ...
    case ActionTypes.SAVE_PROJECT_SUCCESS:
      return [
        ...
        ...state.filter(project => project.id !== (action as any).payload.id)
      ];
    case ActionTypes.DELETE_PROJECT_SUCCESS:
      return [...state.filter(project => project.id !== (action as any).payload.id)];
    case ActionTypes.SAVE_CONNECTION_SUCCESS:
      newState = state.map(project => {
        const updatedProject = { ...project };
        if (project.sourceConnection.id === (action as any).payload.id) {
          updatedProject.sourceConnection = { ...(action as any).payload };
        }
