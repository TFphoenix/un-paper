...
// import * as BrowserFileType from 'file-type/browser';

const BrowserFileType = require('file-type/browser');
...
export class AssetService {
  ...
  getAssetPredictMetadata(asset: IAsset, predictResults: any): IAssetMetadata {
    ...
    const getLabelValues = (field: any) => {
      return field.elements?.map((path: string): IFormRegion => {
        ...
        const word = pathArr.reduce((obj: any, key: string) => obj[key], {
  ...
  public async save(
    ...
    if (
      needCleanEmptyLabel &&
      !metadata.labelData?.labels?.find(label => label?.value?.length !== 0)
    ) {
      cleanLabel = true;
    }
    ...
    const labelTransformer = (labelData: ILabelData) => {
      if (tagType === FieldType.Object || tagType === FieldType.Array) {
        labelData.labels = labelData.labels.filter(
          label => label.label.split('/')[0].replace(/~1/g, '/').replace(/~0/g, '~') !== tagName
        );
      } else {
        if (constants.supportedLabelsSchemas.has(labelData?.$schema)) {
          labelData.labels = labelData.labels.filter(
            label => label.label.replace(/~1/g, '/').replace(/~0/g, '~') !== tagName
          );
        } else {
    ...
    const transformer = (tagNames, columnKey, rowKey) => {
      ...
      if (tagNames[0] === originalTagName) {
        const hasDeletedRowOrKey =
          deletedColumns?.find(deletedColumn => deletedColumn.originalName === columnKey) ||
          deletedRows?.find(deletedRow => deletedRow.originalName === rowKey);
        ...
        const columnRenamed = newColumns?.find(
          newColumn =>
            newColumn.originalName === columnKey && newColumn.originalName !== newColumn.name
        );
        const rowRenamed = newRows?.find(
          newRow => newRow.originalName === rowKey && newRow.originalName !== newRow.name
        );
        if (columnRenamed) {
          ...
          if (tagType === FieldType.Object) {
            ...
            if (visualizationHint === TableVisualizationHint.Vertical) {
              result.push({
                ...label,
                label:
                  tagName.replace(/~/g, '~0').replace(/\//g, '~1') +
                  '/' +
                  (row?.name || rowKey).replace(/~/g, '~0').replace(/\//g, '~1') +
                  '/' +
                  (column?.name || columnKey).replace(/~/g, '~0').replace(/\//g, '~1')
              });
            } else {
              result.push({
                ...label,
            ...
            result.push({
              ...
              label:
                tagName.replace(/~/g, '~0').replace(/\//g, '~1') +
                '/' +
                rowKey.replace(/~/g, '~0').replace(/\//g, '~1') +
                '/' +
                (column?.name || columnKey).replace(/~/g, '~0').replace(/\//g, '~1')
            });
          }
    ...
    if (tagType === FieldType.Array || tagType === FieldType.Object) {
      if (assetMetadata?.labelData?.labels) {
        const field = assetMetadata.labelData.labels.find(
          field => field.label.split('/')[0].replace(/~1/g, '/').replace(/~0/g, '~') === tagName
        );
        if (field) {
          foundTag = true;
  ...
  private reconfigureTableTagInAssetMetadata(
    ...
    if (tagType === FieldType.Array || tagType === FieldType.Object) {
      const field = assetMetadata?.labelData?.labels?.find(
        field =>
          field.label.split('/')[0] === originalTagName.replace(/~/g, '~0').replace(/\//g, '~1')
      );
      if (field) {
        foundTag = true;
      }
  ...
  public static checkAndUpdateSchema = async (project: IProject): Promise<IProject> => {
    ...
    const shouldSchemaUpdate = schema =>
      constants.supportedLabelsSchemas.has(schema) && schema !== constants.labelsSchema;
    if (_.isPlainObject(assets)) {
      const assetService = new AssetService(project);
      const assetMetadatas: IAssetMetadata[] = await Promise.all(
        Object.values(assets).map(async asset => await assetService.getAssetMetadata(asset))
      );
      await Promise.all(
        assetMetadatas.map(async assetMetadata => {
          if (
            _.isPlainObject(assetMetadata.labelData) &&
            AssetService.shouldSchemaUpdate(assetMetadata.labelData?.$schema)
          ) {
